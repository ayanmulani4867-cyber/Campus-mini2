...
def apply_leave():
    ...
    try:
        start_d = date.fromisoformat(str(data["startDate"])[:10])
        end_d = date.fromisoformat(str(data["endDate"])[:10])
    ...
    leave_req = LeaveRequest(
        ...
        leave_type=data.get("leaveType", "Personal"),
        start_date=start_d,
        end_date=end_d,
...
def update_leave_status(leave_id):
    ...
    if "remarks" in data:
        leave_req.review_remarks = str(data["remarks"]).strip()
